...
//           case "pending":
//             setMessage(
//               "Your store request is pending, please wait for admin to approve"
//             );
//             break;
//         }
//       } else {
...
//     if (alreadySubmitted) {
//       toast.info("You have already submitted a store request");
//       return;
//     }
...
//       success: (data) => {
//         return data.message || "Applied, awaiting for approval";
//       },
...
//           Please <span className="text-primary">Login</span> to continue
//         </h1>
//       </div>
//     );
//   }
...
export default function CreateStorePage() {
  ...
  const [status, setStatus] = useState<
    "PENDING" | "APPROVED" | "REJECTED" | "NOT_REGISTERED"
  >("NOT_REGISTERED");
  ...
  const fetchSellerStatus = useCallback(async () => {
    ...
    try {
      ...
      if (data?.data?.storeId) {
        setApplicationId(data.data.storeId);
  ...
  const notifyStoreCreation = (promise: Promise<any>) => {
    toast.promise(promise, {
      loading: "Submitting data...",
      success: (data) => {
        return data.message || "Store application submitted successfully!";
      },
      error: (err) => err.message,
    });
  };
      ...
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="text-center">
          ...
          <p className="text-gray-600">
            Verifying your store application details...
          </p>
        </div>
      </div>
    );
